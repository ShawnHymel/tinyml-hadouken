...
threshold = 0.8         # Softmax value for 'hadouken' needs to be over this
holdoff_time = 1.5      # Time (sec) before another 'hadouken' can trigger
...
maf_buf = np.zeros((maf_pts,))      # Moving average filter buffer
        ...
        if g_flag == 1:
            ...
            if fft_cnt >= ffts_per_inference:
                ...
                if in_holdoff == 1:
                    ...
                    # Push 'hadouken' inference output value to buffer
                    maf_buf[:-1] = maf_buf[1:]
                    maf_buf[-1] = val

                    # Get average
                    avg = np.sum(maf_buf) / maf_pts
